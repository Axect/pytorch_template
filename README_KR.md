...
동일한 바이너리로 Optuna SQLite 데이터베이스를 실시간으로 읽어 HPO 진행 상황을 모니터링할 수 있습니다:

```bash
python -m cli monitor --hpo   # .db 파일 자동 감지
```

4개 탭: **Overview** (trial 산점도 + best 수렴 곡선), **Parameters** (파라미터별 산점도 격자), **Best Trial** (현재 최고 trial의 학습 곡선), **Trials** (대화형 테이블 — 행 선택 후 `Enter`로 해당 trial의 학습 곡선 확인).

| 키 | 동작 |
|----|------|
| `+` / `-` | Y축 줌 인/아웃 |
| `↑` / `↓` | Y축 팬 (Trials 탭에서는 행 선택) |
| `r` | Y축 자동 범위 리셋 |
| `x` | X축 로그 스케일 토글 (Parameters 탭) |
